...
var (
	...
	testcases        = flag.String("testcases", "", "Test cases file for gosecurity functional tests")
)
...
func runIndividualTestCase(testcase string, debug bool) error {
	...
	if err != nil {
		return errors.Wrap(err, "could not run gosecurity main test")
	}
	devOutput, err := testutils.RunNucleiBinaryAndGetLoadedTemplates(*devNucleiBinary, debug, finalArgs)
	if err != nil {
		return errors.Wrap(err, "could not run gosecurity dev test")
	}
