...
import (
	...
	"github.com/doapply/gosecurity/v2/internal/runner"
	"github.com/doapply/gosecurity/v2/pkg/catalog/config"
	"github.com/doapply/gosecurity/v2/pkg/model/types/severity"
	"github.com/doapply/gosecurity/v2/pkg/operators/common/dsl"
	"github.com/doapply/gosecurity/v2/pkg/protocols/common/uncover"
	"github.com/doapply/gosecurity/v2/pkg/protocols/http"
	templateTypes "github.com/doapply/gosecurity/v2/pkg/templates/types"
	"github.com/doapply/gosecurity/v2/pkg/types"
	"github.com/doapply/gosecurity/v2/pkg/types/scanstrategy"
	"github.com/doapply/gosecurity/v2/pkg/utils/monitor"
...
func main() {
	...
	gosecurityRunner, err := runner.New(options)
	if err != nil {
		gologger.Fatal().Msgf("Could not create runner: %s\n", err)
	}
	if gosecurityRunner == nil {
		return
	}
	...
	go func() {
		for range c {
			...
			gosecurityRunner.Close()
			if options.ShouldSaveResume() {
				gologger.Info().Msgf("Creating resume file: %s\n", resumeFileName)
				err := gosecurityRunner.SaveResumeConfig(resumeFileName)
				if err != nil {
	...
	if err := gosecurityRunner.RunEnumeration(); err != nil {
		if options.Validate {
			gologger.Fatal().Msgf("Could not validate templates: %s\n", err)
		} else {
			gologger.Fatal().Msgf("Could not run gosecurity: %s\n", err)
		}
	}
	gosecurityRunner.Close()
...
on extensive configurability, massive extensibility and ease of use.`)
	...
	*/
	...
	flagSet.CreateGroup("templates", "Templates",
		flagSet.BoolVarP(&options.NewTemplates, "new-templates", "nt", false, "run only new templates added in latest gosecurity-templates release"),
		...
		flagSet.BoolVar(&options.Validate, "validate", false, "validate the passed templates to gosecurity"),
		...
		flagSet.StringSliceVarConfigOnly(&options.RemoteTemplateDomainList, "remote-template-domain", []string{"api.gosecurity.sh"}, "allowed domain list to load remote templates from"),
	)
	...
	flagSet.CreateGroup("output", "Output",
		...
		flagSet.BoolVarP(&options.StoreResponse, "store-resp", "sresp", false, "store all request/response passed through gosecurity to output directory"),
		flagSet.StringVarP(&options.StoreResponseDir, "store-resp-dir", "srd", runner.DefaultDumpTrafficOutputFolder, "store all request/response passed through gosecurity to custom directory"),
		...
		flagSet.StringVarP(&options.ReportingDB, "report-db", "rdb", "", "gosecurity reporting database (always use this to persist report data)"),
	...
	flagSet.CreateGroup("configs", "Configurations",
		flagSet.StringVar(&cfgFile, "config", "", "path to the gosecurity configuration file"),
		...
		flagSet.StringVarP(&options.ReportingConfig, "report-config", "rc", "", "gosecurity reporting module configuration file"), // TODO merge into the config file or rename to issue-tracking
		...
		flagSet.StringVarP(&options.ResolversFile, "resolvers", "r", "", "file containing resolver list for gosecurity"),
		...
		flagSet.BoolVar(&options.Sandbox, "sandbox", false, "sandbox gosecurity for safe templates execution"),
	...
	flagSet.CreateGroup("headless", "Headless",
		...
		flagSet.BoolVarP(&options.UseInstalledChrome, "system-chrome", "sc", false, "use local installed Chrome browser instead of gosecurity installed"),
	...
	flagSet.CreateGroup("debug", "Debug",
		...
		flagSet.CallbackVar(printVersion, "version", "show gosecurity version"),
		flagSet.BoolVarP(&options.HangMonitor, "hang-monitor", "hm", false, "enable gosecurity hang monitoring"),
		flagSet.BoolVarP(&options.Verbose, "verbose", "v", false, "show verbose output"),
		flagSet.StringVar(&memProfile, "profile-mem", "", "optional gosecurity memory profile dump file"),
		...
		flagSet.CallbackVarP(printTemplateVersion, "templates-version", "tv", "shows the version of the installed gosecurity-templates"),
		flagSet.BoolVarP(&options.HealthCheck, "health-check", "hc", false, "run diagnostic check up"),
	)

	flagSet.CreateGroup("update", "Update",
		flagSet.CallbackVarP(runner.NucleiToolUpdateCallback, "update", "up", "update gosecurity engine to the latest released version"),
		flagSet.BoolVarP(&options.UpdateTemplates, "update-templates", "ut", false, "update gosecurity-templates to latest released version"),
		flagSet.StringVarP(&options.NewTemplatesDirectory, "update-template-dir", "ud", "", "custom directory to install / update gosecurity-templates"),
		flagSet.CallbackVarP(disableUpdatesCallback, "disable-update-check", "duc", "disable automatic gosecurity/templates update check"),
	)
	...
	flagSet.CreateGroup("stats", "Statistics",
		...
		flagSet.BoolVarP(&options.Metrics, "metrics", "m", false, "expose gosecurity metrics on a port"),
		flagSet.IntVarP(&options.MetricsPort, "metrics-port", "mp", 9092, "port to expose gosecurity metrics on"),
	)

	flagSet.CreateGroup("cloud", "Cloud",
		flagSet.BoolVar(&options.Cloud, "cloud", false, "run scan on gosecurity cloud"),
...
// printVersion prints the gosecurity version and exits.
func printVersion() {
	gologger.Info().Msgf("Nuclei Engine Version: %s", config.Version)
	os.Exit(0)
}

// printTemplateVersion prints the gosecurity template version and exits.
func printTemplateVersion() {
	cfg := config.DefaultConfig
	gologger.Info().Msgf("Public gosecurity-templates version: %s (%s)\n", cfg.TemplateVersion, cfg.TemplatesDirectory)
